    ...
    <div className={`${darkMode ? "bg-gray-900 text-gray-200" : "bg-gray-100 text-gray-800"} min-h-screen transition-colors duration-500 font-sans`}>
      ...
      <header className="flex justify-between items-center px-6 py-4 shadow-md sticky top-0 bg-red-500 dark:bg-red-800 z-50">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-2xl font-extrabold text-white">Tickitie</h1>
        </div>

        <button
          onClick={() => setDarkMode(!darkMode)}
          className="bg-white text-red-600 dark:bg-gray-700 dark:text-gray-200 px-4 py-2 rounded-md shadow hover:bg-gray-100 dark:hover:bg-gray-600 transition-all flex items-center gap-2"
      ...
      <main className="flex flex-col items-center py-12 px-4 max-w-5xl mx-auto space-y-12 w-full">
        {/* Welcome */}
        ...
        {/* Navigation */}
        <nav className="flex gap-4 flex-wrap justify-center text-sm">
          {!user ? (
            <>
              <Link
                href="/signin"
                className="px-5 py-2 rounded-full bg-green-500 text-white hover:bg-green-600 shadow transition-all font-semibold"
              >
                Sign In
              </Link>
              <Link
                href="/signup"
                className="px-5 py-2 rounded-full bg-purple-500 text-white hover:bg-purple-600 shadow transition-all font-semibold"
              >
                Sign Up
              </Link>
            </>
            ...
            <>
              <Link
                href="/log-ticket"
                className="px-5 py-2 rounded-full bg-blue-500 text-white hover:bg-blue-600 shadow transition-all font-semibold"
              >
                Log Ticket
              </Link>
              {isTechnician && (
                <Link
                  href="/technician"
                  className="px-5 py-2 rounded-full bg-indigo-500 text-white hover:bg-indigo-600 shadow transition-all font-semibold"
                >
          ...
          <div className="grid gap-4">
            ...
            {news.map((article, idx) => (
              <a
                ...
                className="block p-4 rounded-xl shadow-md bg-white hover:bg-red-100 dark:bg-gray-800 dark:hover:bg-gray-700 transition-all"
              >
